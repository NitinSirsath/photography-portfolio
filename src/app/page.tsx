...
export default async function Home() {
  // 1. Identify logged in user (Optional)
  ...
  // 2. Query the global ecosystem with Appreciation counts
  const globalArtworks = await prisma.artwork.findMany({
    ...
    include: {
      ...
      likes: true // Include appreciation records
    }
  })
        ...
        {globalArtworks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 auto-rows-[450px]">
             {globalArtworks.map((item) => {
                ...
                const isLikedByMe = authUser ? item.likes.some(l => l.userId === authUser.id) : false
