...
export default function LoginPage() {
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        // This will bounce back to our callback route which captures the secure cookie
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      ...
      >
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-4 bg-foreground text-background py-4 rounded-xl hover:scale-[0.98] transition-transform shadow-lg"
        >
